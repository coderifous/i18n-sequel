...
class I18nSequelMissingTest < I18n::TestCase
  class Backend < I18n::Backend::Sequel
    include I18n::Backend::Sequel::Missing
  end
  ...
  test "can persist interpolations" do
    translation = I18n::Backend::Sequel::Translation.new(:key => 'foo', :value => 'bar', :locale => :en)
  ...
  test "lookup persists the key" do
    I18n.t('foo.bar.baz')
    assert_equal 1, I18n::Backend::Sequel::Translation.count
    assert I18n::Backend::Sequel::Translation.locale(:en).find_by_key('foo.bar.baz')
  end
  ...
  test "lookup persists interpolation keys when looked up directly" do
    ...
    translation_stub = I18n::Backend::Sequel::Translation.locale(:en).lookup('foo.bar.baz').first
    assert translation_stub.interpolates?(:cow)
  end

  test "creates one stub per pluralization" do
    I18n.t('foo', :count => 999)
    translations = I18n::Backend::Sequel::Translation.locale(:en).where key: %w{ foo.zero foo.one foo.other }
    assert_equal 3, translations.count
  end

  test "creates no stub for base key in pluralization" do
    I18n.t('foo', :count => 999)
    assert_equal 3, I18n::Backend::Sequel::Translation.locale(:en).lookup("foo").count
    assert !I18n::Backend::Sequel::Translation.locale(:en).find_by_key("foo")
  end

  test "creates a stub when a custom separator is used" do
    I18n.t('foo|baz', :separator => '|')
    I18n::Backend::Sequel::Translation.locale(:en).lookup("foo.baz").first.update(:value => 'baz!')
  ...
  test "creates a stub per pluralization when a custom separator is used" do
    ...
    translations = I18n::Backend::Sequel::Translation.locale(:en).where key: %w{ foo.bar.zero foo.bar.one foo.bar.other }
    assert_equal 3, translations.count
  end
  ...
  test "creates a stub when a custom separator is used and the key contains the flatten separator (a dot character)" do
    ...
    I18n::Backend::Sequel::Translation.locale(:en).lookup("foo.baz\001zab").first.update(:value => 'baz!')
